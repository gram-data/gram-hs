...
As a developer working with patterns, I need to perform computations that have access to the full structural context (parent, siblings, depth, path) around each value, so that I can compute results based on position and structure, not just the value itself.
...
3. **Given** a pattern and a function that computes the path from root at each position, **When** I call `extend` with the function, **Then** I get a pattern where each position contains its path from root
...
5. **Given** context-aware functions of various types (depth, size, path, custom computations), **When** I verify comonad laws hold, **Then** all laws pass for all function types

---

### User Story 5 - Context-Aware Operations (Priority: P2)

As a developer working with patterns, I need convenient helper functions for common context-aware operations (depth, size, path) so that I can easily compute structural metadata at each position without manually implementing context-aware functions.

**Why this priority**: While context-aware operations are useful, they are secondary to the core Comonad instance (User Stories 1-4). Helper functions like `depthAt`, `sizeAt`, and `pathAt` provide convenience but can be implemented using `extend` with custom functions. The core Comonad instance must be complete and verified before adding convenience functions.
...
3. **Given** a pattern structure, **When** I call `pathAt` on it, **Then** I get a pattern where each position contains its path from root (list of indices)
4. **Given** an atomic pattern, **When** I call context-aware operations, **Then** operations return appropriate results (depth 0, size 1, path [])
...
- What happens when `extend` is called with functions that access structural information (depth, size, path) at each position?
...
- **FR-014**: System MUST provide tests verifying extend with various context-aware functions (depth, size, path, custom computations)
...
- **FR-019**: System SHOULD provide helper functions for common context-aware operations: `pathAt :: Pattern v -> Pattern [Int]` (path from root at each position)
...
- **Context**: The full structural information available at a position in a pattern, including the pattern structure focused at that position, parent context, sibling context, depth, path, and size.
- **Context-Aware Function**: A function that takes a Pattern and returns a result based on the full structural context at that position, not just the value. Examples include functions that compute depth, size, path, or custom structural metadata.
...
- **SC-002**: Developers can create context-aware computations using `extend`, enabling transformations based on structural context (depth, size, path, relationships)
...
- **SC-010**: Context-aware helper functions (depthAt, sizeAt, pathAt) produce correct results for all pattern structures with 100% success rate (if implemented)
...
- Context-aware functions will have access to the full pattern structure at each position, enabling computations based on depth, size, path, parent, siblings, and other structural information
...
- Context-aware operations (depthAt, sizeAt, pathAt) are optional convenience functions that can be implemented using `extend` with custom functions
...
- Comonad enables context-aware folding where computations have access to the full structural context (parent, siblings, depth, path) around each value, not just the value itself. This extends beyond `Foldable` which only provides values in sequence.
...
- Context-aware operations (depthAt, sizeAt, pathAt) are optional convenience functions that demonstrate the power of the Comonad instance but are not required for the core Comonad functionality.
